...
cache_create(char *name,		/* name of the cache */
  ...
  /* print derived parameters during debug */
   		...
   		debug("%s: cp->name %s", cp->name, cp->name);
   		debug("%d: cp->nset %s", cp->nsets, cp->name);
   		debug("%d: cp->bsize %s", cp->bsize, cp->name);

   		// debug("%s: cp->name", cp->na)
...
cache_reg_stats(struct cache_t *cp,	/* cache instance */
  ...
  /* get a name for this cache */
      ...
      // sprintf(buf, "%s Hello", name);
		...
		stat_reg_counter(sdb, buf, "total number of victim cahce hits", &cp->vc_hits, 0, NULL);
		

		// sprintf(buf, "%s.victim_misses", name);
		// stat_reg_counter(sdb, buf, "total number of victim cache misses", &cp->vc_misses, 0, NULL);
		
		// victim cache end

		// miss cache start
		sprintf(buf, "%s.misscache_hits", name);
		stat_reg_counter(sdb, buf, "total number of victim cahce hits", &cp->mc_hits, 0, NULL);
		

		// sprintf(buf, "%s.misscache_misses", name);
		// stat_reg_counter(sdb, buf, "total number of victim cache misses", &cp->mc_misses, 0, NULL);
		// miss cache end

		// selective cache
		sprintf(buf, "%s.selective_hits", name);
		stat_reg_counter(sdb, buf, "total number of selective cahce hits", &cp->sel_hits , 0, NULL);
		

		// sprintf(buf, "%s.selective_misses", name);
		// stat_reg_counter(sdb, buf, "total number of selective cache misses", &cp->sel_misses, 0, NULL);
		// selective cache end
...
cache_access(struct cache_t *cp,	/* cache to access */
 ...
 cache_hit: /* slow hit handler */
   	// update hit flag
  /* **HIT** */
   	if( cp->name == 'dl1' || cp->name == 'il1')
	   	set_sticky(cp, addr, 1);

	  if( cp->name == 'dl2' || cp->name == 'il2')
	   	set_hit(cp, addr, 1);
   	// int f = cp->blk_access_fn(Read, CACHE_BADDR(cp, addr), cp->bsize,
   	// 	repl, now+lat);
   	cp->hits++;
   	cp->last_hit = 1;
